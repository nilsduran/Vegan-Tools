...
| **MAP-06** | **Manca de títols al mapa**: Els usuaris només veien xinxetes anònimes sense saber quin restaurant era quin. | Els marcadors no tenien etiquetes de text permanents. | ✅ **RESOLT** | Afegit `marker.bindTooltip(restaurant.name, { permanent: true, direction: "bottom", className: "map-pin-name-tooltip" })`. |
...
| **MAP-08** | **Botó de netejar cerca (X) invisible**: La creu d'esborrar text de cerca apareixia com un requadre gris buit. | SVG sense contrast ni stroke definit en heretar colors de formulari. | ✅ **RESOLT** | Estilitzat amb fons `#f3f4f6`, vora clara, icona centrada de 14x14px i stroke `2.5`. |
